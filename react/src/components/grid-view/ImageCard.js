...
import mock_images from '../../assets/txt/ffind-defaults/mock_images.json'
    ...
    if (this.state.loaded) {
      ...
      return (<
        ...
        <
          ...
          <
            CardImg className='image-card-thumb'
            top width='100%'
            src={`${process.env.REACT_APP_IMAGE_URL}/${this.props.iid}.jpg`}
            alt={this.state.imgName}
          /> <
            FontAwesomeIcon icon='search-plus'
            size='3x'
            className='card-search-plus' />
          <
            /div> <
            ...
            div className='marker-list' > {
                            Object.keys(this.state.markers).slice(0, Object.keys(this.state.markers).length - 1).map(marker => (<
                              MarkerTag filterActive={this.props.filterActive}
                              key={`${marker.replace(' ', '_').replace(/[^0-9a-zA-Z\-_]/gi, '')}`}
                              filterCallback={this.props.filterCallback}
                              marker={marker}
                              iid={this.props.iid}
                              color={this.state.markerColors[marker.toUpperCase()]}
                            />
                            ...
                            (Object.keys(this.state.markers).length > 0 && Object.keys(this.state.markers)[0] !== 'DEFAULT VAL') ? (<
                              MarkerTag filterActive={this.props.filterActive}
                              key={`${this.state.markers[lastMarker].replace(' ', '_').replace(/[^0-9a-zA-Z\-_]/gi, '')}`}
                              filterCallback={this.props.filterCallback}
                              marker={lastMarker}
                              iid={this.props.iid}
                              color={this.state.markerColors[lastMarker.toUpperCase()]}
                            />
                            ) : null
...

            {
                    ...
                    span className='tag'
                                      key={`${this.props.iid}-${item.replace(' ', '_')}`} > < span > {' ' + item} < /span><span>, </span > < /span>
                ))
            } <
            span className='tag'
                                          key={this.props.iid + this.state.imgTags[this.state.imgTags.length - 1]} > {this.state.imgTags[this.state.imgTags.length - 1]} < /span> < /
            div > { /* <a href={this.props.path_path} target="_blank"><Button color="link" className="mt-auto">View More Info</Button></a> */} <
            /CardBody> <
            CardFooter >
                                            <
            Row >
            ...
            Button id='modal-button'
                                                  ...
                                                  onClick={
                                                    () => this.props.callback(this.props.iid)
                                                  } > Preview < FontAwesomeIcon icon='search-plus'
                                                    size='1x' /> < /Button> {
                                                    this.props.isFavorite && < Button id='save-button'
                                                      outline color='info'
                                                      className='favorite'
                                                      onClick={
                                                        () => this.props.favoriteCallback(this.props.iid)
                                                      } > Save < FontAwesomeIcon icon={
                                                        ['far', 'bookmark']
                                                      }
                                                        size='1x' /> < /Button>} {!this.props.isFavorite && < Button id='unsave-button'
                                                          outline color='danger'
                                                          className='favorite'
                                                          onClick={
                                                            () => this.props.favoriteCallback(this.props.iid)
                                                          } > Remove < FontAwesomeIcon icon={
                                                            ['fas', 'bookmark']
                                                          }
                                                            size='1x' /> < /Button>} < /
            div > <
